...
__global__ void dev_overrelax_step(dev_su3_2v * trafo_new, dev_su3_2v * gf, dev_su3_2v * trafo, int * dev_indeo_thissite, int * dev_indeo_nextside, int * dev_nn){
 ...
 //positive dir
      ...
      //gauge_field  U_mu(x)
      #ifdef GF_8
        dev_reconstructgf_8texref(gf, (4*pos+mu),&(gfsmem));
      #else
        dev_reconstructgf_2vtexref(gf, (4*pos+mu),&(gfsmem));
      #endif
      //trafo_field  g^+(x+mu)
      #ifdef GF_8
        dev_reconstructtrafo_8texref_dagger(trafo, hoppos,&(trafosmem));
      #else
        dev_reconstructtrafo_2vtexref_dagger(trafo, hoppos,&(trafosmem));
      #endif
      
      
      //dev_su3_ti_su3( &(gt), &(gfsmem), &(trafosmem) );    
      //dev_su3_add( &(star), &(gt));
      dev_add_su3_ti_su3(&(star) , &(gfsmem), &(trafosmem) );
      
  //negative dir
      hoppos = dev_nn[8*pos+4+mu];
      //gauge_field  U_mu(x-mu)^+
      #ifdef GF_8
       dev_reconstructgf_8texref_dagger(gf, 4*hoppos+mu,&(gfsmem));
      #else
       dev_reconstructgf_2vtexref_dagger(gf, 4*hoppos+mu,&(gfsmem));
      #endif
    ...
    //load g(x)
      #ifdef GF_8
        dev_reconstructtrafo_8texref(trafo, pos,&(trafosmem));
      #else
        dev_reconstructtrafo_2vtexref(trafo, pos,&(trafosmem));
      #endif
   ...
   */
   ...
   overrelax(&(trafosmem), &(help), 1.68, 3);    
   dev_su3_normalize( &(trafosmem) ); 
   
   #ifdef GF_8
     dev_storetrafo_8(pos, trafo_new ,&(trafosmem) );
   #else
     dev_storetrafo_2v(pos, trafo_new ,&(trafosmem) );
   #endif 
...
for(i=0; i<nrep; i++){
 ...
 // x2 because 2x overrelaxstep per iteration
  double benchres = 4766.0*2*(VOLUME/2)* nrep / timeelapsed / 1.0e9;
