...
double * dev_redfield_F;
double * dev_redfield_dAdA;
double * dev_redfield_maxdAdA;
double * dev_redfield_plaq;
...
while ((c = getopt(argc, argv, "h?:i:")) != -1) {
  ...
  if(thermflag==1){
    ...
    init_RNG(4*VOLUME/2, 4*4*VOLUME/2, randseed); // we need 4 sets of (1/4) (gauss/unif) numbers
  ...
  init_RNG(VOLUME/2, 4*VOLUME/2, randseed); // need one gauss rnd and 4 unif. rnd for all lattice points 
